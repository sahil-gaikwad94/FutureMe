...
export default function Home() {
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ role: "user" | "assistant"; content: string }>>([{ role: "assistant", content: "You’re not behind. You’re at the part where small, repeated choices begin to look like a life. What should we look at together?" }]);
  ...
  const sendChat = trpc.workspace.chat.useMutation({ onSuccess: data => setChatMessages(items => [...items, { role: "assistant", content: data.response }]), onError: () => setChatMessages(items => [...items, { role: "assistant", content: "The projection is still here. I could not reach the reflection layer just now — try again in a moment." }]) });
  ...
  const handleSend = () => {
    ...
    else setTimeout(() => setChatMessages(items => [...items, { role: "assistant", content: "Sign in and I’ll carry this question into your own projection. For now, move the consistency dial — you can watch the answer change." }]), 450);
  };
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#080a16] text-white selection:bg-[#ff9f7a]/30">
    ...
    <main className="relative z-10 mx-auto max-w-[1480px] px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
      ...
      <section className="mt-16 grid gap-5 xl:grid-cols-[1fr_.95fr]">
